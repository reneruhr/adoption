struct graph_canvas
{
	...
	graph_canvas(vec2_f32 size, vec2_f32 pos, arena& arena) : size(size), pos(pos)
	{
		...
		{.binding = 3, .size = max_plots * 1 * sizeof(per_frame_data) ,.data = 0},
		};
		...
		{.binding = 3, .size = max_quads * 1 * sizeof(per_frame_data) ,.data = 0},
		};
		...
		{.binding = 3, .size = max_pointset * 1 * sizeof(per_frame_data) ,.data = 0},
		};
		...
		{.binding = 3, .size = max_shapes   *  sizeof(per_frame_data2) ,.data = 0},
	...
	glTextureParameteri(context_shapes.tex, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTextureParameteri(context_shapes.tex, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
