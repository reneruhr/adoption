...
opengl_context context_from_colored_mesh(mesh mesh)
{
...
image draw(func_2d f, u32 n)
{
	u8* data = new u8[n*n*4];
	for (u32 i{}; i < n; i++)
		for (u32 j{}; j < n; j++)
		{
			f32 x = 2*(f32)i / n - 1.;
			...
			f32 r = x * x + y * y;
			u8 fval = 255u * f(x, y);
			data[4 * (j + i * n  )] = r <= 1.f ? fval : 100;
			data[4*(j + i * n) + 1] = r <= 1.f ? fval : 0;
			data[4*(j + i * n) + 2] = r <= 1.f ? fval : 0;
			data[4*(j + i * n) + 3] = r <= 1.f ? 255u : 100u;
		}
	return { data, n * n };
}

opengl_context context_from_func_2d(mesh mesh,func_2d f, u32 size)
{
	opengl_context context_mesh
	{
		.program		= compile_shaders( R"(
